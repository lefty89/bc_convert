...
class ConvertUtility
{
    ...
    public static function isConvertionFinished($queue)
    {
        ...
        (array_key_exists('progress',
                $data) && (intval($data['progress']) === 100));    // check whether the progress is at 100 percent
    }
    ...
    public static function startConvertingVideo(&$queue)
    {
        ...
        $outputFile = $queue->getPath() ?: FileUtility::createTempFileInFolder(pathinfo($queue->getFile()->getName(),
            PATHINFO_FILENAME), $queue->getFormat());
